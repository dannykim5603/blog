...
public class DispatcherServlet extends HttpServlet {
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// DB 접속 시작
		String url = "jdbc:mysql://site32.iu.gy:3306/site32?serverTimezone=Asia/Seoul&useOldAliasMetadataBehavior=true&zeroDateTimeBehavior=convertToNull";
		String user = "site32";
		...
		try {
			...
			Util.prinEx("SQL예외 (커넥션 열기)", resp, e);
		} catch (Exception e) {
			Util.prinEx("기타예외", resp, e);
		} finally {
			if (dbConn != null) {
				try {
					dbConn.close();
				} catch (SQLException e) {
					Util.prinEx("[SQLException 예외(커넥션 닫기)", resp ,e);
				}
			}
		}
	}
